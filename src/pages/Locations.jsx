...
export default function Locations() {
  ...
  // Trava o scroll da página de fundo enquanto o modal de detalhes está
  // aberto -- sem isso dava pra rolar Locações "por trás" do modal ao mesmo
  // tempo, além do próprio modal não ter área de rolagem interna.
  useEffect(() => {
    if (!viewOpen) {
      return
    }
    ...
    <div className="space-y-8">
      ...
      {viewOpen && selectedLocation ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 px-4 py-6">
          <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-[32px] border border-white/10 bg-slate-950 shadow-2xl shadow-black/50">
            <div className="sticky top-0 z-10 flex shrink-0 items-start justify-between gap-4 border-b border-white/10 bg-slate-950 p-4 sm:p-6">
              ...
              <button type="button" onClick={() => setViewOpen(false)} className="shrink-0 rounded-full border border-white/10 px-3 py-2 text-sm text-slate-300">Fechar</button>
            </div>
            ...
            <div className="sticky bottom-0 shrink-0 border-t border-white/10 bg-slate-950 p-4 sm:p-6 flex justify-end">
              <button
                type="button"
                onClick={() => {
