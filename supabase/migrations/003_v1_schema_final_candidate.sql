-- CANDIDATO FINAL PARA REVISÃO
-- Objetivo: versão final candidata da migração 002, sem executar no Supabase.
-- Mantém compatibilidade com as tabelas atuais e preserva históricos.
...
-- =========================================================
-- 1) PERFIS E ENTIDADES DE NEGÓCIO
-- =========================================================
...
-- =========================================================
-- 2) PAGAMENTOS FINANCEIROS (MODELOS PARTNERS / SAVINGS)
-- =========================================================
...
create table if not exists public.rental_payments (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint ck_rental_payments_financial_rule
    check (
...
begin
  ...
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'audit_logs' and column_name = 'previous_values'
  ) then
  ...
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'audit_logs' and column_name = 'is_deleted'
  ) then
  ...
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'audit_logs' and column_name = 'deleted_at'
  ) then
...
-- =========================================================
-- 7) FUNÇÕES E TRIGGERS IDÊNTICOS E IDÊMPOTENTES
-- =========================================================
...
-- =========================================================
-- 8) RLS INICIAL: SOMENTE AUTHENTICATED
-- =========================================================
